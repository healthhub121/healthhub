...
function generateTimeSlots(fromHour = 9, fromMin = 0, toHour = 17, toMin = 0) {
    const slots = [];
    // Safety check
    if (toHour < fromHour || (toHour === fromHour && toMin <= fromMin)) return slots;
    let hour = fromHour, minute = fromMin;
    let count = 0;
    while ((hour < toHour || (hour === toHour && minute < toMin)) && count < 200) {
...
function openBookingModal(docId) {
    ...
    // تحميل الأوقات المتاحة فوراً عند فتح المودال (هي اللي بتملأ الـ select)
...
async function updateAvailableSlots() {
    ...
    try {
        ...
        if (!daySchedule || daySchedule.isClosed) {
            timeSelect.innerHTML = `<option value="" disabled>اختر يوماً آخر</option>`;
            timeAvailability.textContent = `🏖️ يوم ${dayName} إجازة، اختر يوماً آخر`;
            timeAvailability.style.color = '#e74c3c';
            return;
        }
        ...
        // توليد الأوقات
        let slots;
        if (daySchedule && (daySchedule.start || daySchedule.from) && (daySchedule.end || daySchedule.to)) {
            const fromStr = daySchedule.start || daySchedule.from;
            const toStr = daySchedule.end || daySchedule.to;
