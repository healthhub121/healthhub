...
// اليوم المختار حالياً
// Map JS getDay() to our daysOfWeek array index
// JS: 0=Sun,1=Mon,2=Tue,3=Wed,4=Thu,5=Fri,6=Sat
// Array: [السبت,الأحد,الاثنين,الثلاثاء,الأربعاء,الخميس,الجمعة]
const dayMap = [1, 2, 3, 4, 5, 6, 0]; // JS day -> our index
let selectedDay = dayMap[new Date().getDay()];

function renderDayFilterButtons() {
    const container = document.getElementById('day-filter-btns');
    if (!container) return;
    container.innerHTML = '';
    daysOfWeek.forEach((day, index) => {
        const btn = document.createElement('button');
        btn.textContent = day;
        btn.style.cssText = `padding:8px 14px; border-radius:20px; border:2px solid var(--primary); 
            cursor:pointer; font-family:Cairo,sans-serif; font-size:13px; font-weight:600;
            background:${index === selectedDay ? 'var(--primary)' : 'transparent'};
            color:${index === selectedDay ? '#fff' : 'var(--primary)'};
            transition: all 0.2s;`;
        btn.onclick = () => {
            selectedDay = index;
            renderDayFilterButtons();
            renderPatients();
            const title = document.getElementById('patients-list-title');
            if (title) title.textContent = 'قائمة المرضى - ' + day;
        };
...
function savePatientsData() {
    sessionStorage.setItem('healthhub_clinic_patients', JSON.stringify(patients));
}
...
document.getElementById('doc-form')?.addEventListener('submit', async function(e) {
    ...
    if (docMode === 'register') {
        ...
        try {
            ...
            if (data.success) {
                sessionStorage.setItem('currentDoctorName', data.name);
                sessionStorage.setItem('currentDoctorId', data.id);
                if (data.specialty) sessionStorage.setItem('currentDoctorSpecialty', data.specialty);
...
function generateScheduleInputs() {
    ...
    if(!container) return;
    const saved = JSON.parse(sessionStorage.getItem('weeklySchedule')) || {};
    const savedPrice = sessionStorage.getItem('clinicPrice') || '';
...
async function updateDoctorPrice() {
    ...
    try {
        ...
        if (response.ok) {
            const data = await response.json();
            if (data.success) {
                sessionStorage.setItem('clinicPrice', price);
...
async function saveWeeklySchedule() {
    ...
    sessionStorage.setItem('weeklySchedule', JSON.stringify(schedule));
...
function renderPatients() {
    ...
    const filtered = patients.filter(p => {
        ...
        const matchesDay = !p.time ? true : new Date(p.time).getDay() === selectedDay;
...
function addPrescription(patientId) {
    ...
    const doctorName = sessionStorage.getItem('currentDoctorName') || 'الطبيب';
...
function confirmEndDay() {
    const dayPatients = patients.filter(p => !p.time || new Date(p.time).getDay() === selectedDay);
    if(dayPatients.length === 0) return alert("لا يوجد مرضى لهذا اليوم!");
    if(confirm("هل تريد إنهاء يوم " + daysOfWeek[selectedDay] + " وحفظ السجل؟")) {
        let history = JSON.parse(sessionStorage.getItem('clinicHistory')) || [];
        history.push({ 
            date: daysOfWeek[selectedDay] + ' - ' + new Date().toLocaleDateString('ar-EG'),
            count: dayPatients.length,
            data: [...dayPatients] 
        });
        sessionStorage.setItem('clinicHistory', JSON.stringify(history));
        // Remove only this day patients
        patients = patients.filter(p => p.time && new Date(p.time).getDay() !== selectedDay);
        renderPatients();
        alert("تم أرشفة بيانات يوم " + daysOfWeek[selectedDay] + " بنجاح.");
    }
}
...
async function showDashboard(name) {
    ...
    // عرض التخصص المحفوظ
    const specialty = sessionStorage.getItem('currentDoctorSpecialty');
...
async function loadDoctorPhoto() {
    ...
    // 1) عرض فوري من sessionStorage لو موجودة
    ...
    try {
        ...
        if (me && me.profile_pic) {
            sessionStorage.setItem(key, me.profile_pic);
        ...
        console.warn('تعذر جلب صورة الدكتور من السيرفر (محملة من sessionStorage):', err);
    }
}
...
async function saveProfileChanges() {
    ...
    try {
        ...
        const data = await res.json();
        if (res.ok && data.success) {
            sessionStorage.setItem('currentDoctorName', name);
            sessionStorage.setItem('currentDoctorSpecialty', specialty);
            const displayEl = document.getElementById('doc-name-display');
            if (displayEl) displayEl.textContent = name;
            const specEl = document.getElementById('doc-specialty-display');
...
function goToSection(target) {
    ...
    if(target === 'doctor') {
        const savedDoc = sessionStorage.getItem('currentDoctorName');
...
document.addEventListener('DOMContentLoaded', () => {
    initTheme();
    renderDayFilterButtons();
    const docName = sessionStorage.getItem('currentDoctorName');
    const docId   = sessionStorage.getItem('currentDoctorId');
    if(docName) {
        ...
        if (docId) {
            fetch('https://healthhub-production-90ef.up.railway.app/doctor/doctors').then(r => r.json()).then(all => {
                ...
                if (me) {
                    sessionStorage.setItem('currentDoctorName', me.name);
                    if (me.specialty) sessionStorage.setItem('currentDoctorSpecialty', me.specialty);
                }
            }).catch(() => {});
        }
...
async function loadPersonalInfo() {
    ...
    try {
        ...
        if (!res.ok) {
            // fallback محلي
            const saved = sessionStorage.getItem('doctorPersonalInfo');
            if (saved) {
...
async function savePersonalInfo() {
    ...
    sessionStorage.setItem(`certImages_${doctorId}`, JSON.stringify(certImages));
    try {
        ...
        if (res.ok && data.success) {
            ...
            sessionStorage.setItem('doctorPersonalInfo', JSON.stringify(payload));
...
function removeCertImage(idx) {
    ...
    renderCertImages();
    // احفظ فوراً في sessionStorage عشان الحذف يتحفظ حتى لو الصفحة اتعملت refresh
    const doctorId = sessionStorage.getItem('currentDoctorId');
    if (doctorId) {
        sessionStorage.setItem(`certImages_${doctorId}`, JSON.stringify(certImages));
    }
...
function loadCertImages() {
    const doctorId = sessionStorage.getItem('currentDoctorId');
    if (!doctorId) return;
    const saved = sessionStorage.getItem(`certImages_${doctorId}`);
    if (saved) {
...
window.addEventListener('load', function() {
    const adminLoggedIn = sessionStorage.getItem('adminLoggedIn');
    const doctorLoggedIn = sessionStorage.getItem('currentDoctorId');
